...
import { chartLinhaMensal, chartBarsH, chartBarsV, chartDonut } from "../charts/performance";
import { fmtBrl, fmtPct } from "@/lib/format";
import { MESES_ABBR } from "@/lib/meses";
import type { FatRow } from "@/lib/faturamento";

const C = { blue: "#2F3192", orange: "#F5781C", green: "#00B050", red: "#FF3B30", mid: "#6B7280", ink: "#1F2440" };
...
export default function PerformancePage({ data, error }: { data: FatRow[] | null; error: string | null }) {
  ...
  const opts = useMemo(() => ({
    ...
    projetos: uniq((data ?? []).map((r) => r.Projeto)),
    regioes: uniq((data ?? []).map((r) => r.Regiao)),
  ...
  const ins = useMemo(() => {
    ...
    return { total, unidades, medio, crescMed, topUni };
  }, [df]);
  ...
  const mensal = useMemo(() => {
    ...
    for (const r of df) {
      ...
      const cur = m.get(k) ?? { ano: r.Ano, mes: r.Mes, v: 0 }; cur.v += r.Faturamento; m.set(k, cur);
    }
  ...
  const porProjeto = useMemo(() => [...groupSum(df, (r) => r.Projeto, (r) => r.Faturamento).entries()].sort((a, b) => b[1] - a[1]), [df]);
  const porCategoria = useMemo(() => {
    const s = (f: (r: FatRow) => number) => df.reduce((a, r) => a + f(r), 0);
    return [
      ["Locação", s((r) => r.Locacao)], ["Renovação", s((r) => r.Renovacao)], ["Serviços", s((r) => r.Servicos)],
      ["Venda", s((r) => r.Venda)], ["Manutenção", s((r) => r.Manutencao)],
  ...
  const tableRows = ins.topUni.slice(0, 15).map(([u, v]) => ({ uni: u, fat: fmtBrl(v), part: ins.total > 0 ? fmtPct((v / ins.total) * 100) : "—" }));
  ...
  return (
    <div className="viab">
      ...
      <div className="viab-kpis cols-4">
        <Kpi label="Faturamento Total" value={fmtBrl(ins.total)} sub={`${df.length} registros mês`} accent={C.green} icon="coins" />
      ...
      <div className="viab-grid viab-grid-2">
        <Panel span title="Evolução do Faturamento" sub="Soma mensal das unidades" height={340}>
          <PlotlyChart {...chartLinhaMensal(mensal.labels, mensal.values)} height={340} />
        </Panel>
        ...
        <Panel title="Faturamento por Projeto" sub="Por tipo de unidade" height={320}>
          <PlotlyChart {...chartDonut(porProjeto.map(([k]) => k), porProjeto.map(([, v]) => v))} height={320} />
        </Panel>
      </div>
